...
newtype Visor a = Visor [ConvNet] deriving (Show, Generic)
instance Serialize (Visor a)
...
initVisorTrainState :: GameState a => Visor a -> [TrainState]
initVisorTrainState v@(Visor nets) = initTrainState params <$> nets
  where params = defaultParams . config $ v

toVisor states = Visor $ network <$> states

initVisor :: forall a. GameState a => Visor a
initVisor = Visor (fmap widgetNet ws)
  where widgetNet (Widget res _ _ cs spec) = initCNet spec res res ((+1) <$> cs) -- +1 to account for Indeterminate in cardinality
        ws = widgetDfn . config $ (undefined :: p a)


feedVisor :: Monad m => Visor a -> [[Palette]] -> Double -> m [[WidgetLabel]]
...
feedVisorFast :: Monad m => Visor a -> [[Volume]] -> Double -> m [[WidgetLabel]]
...
loadVisor :: GameState a => FilePath -> IO (Visor a)
loadVisor fp = do exist <- doesFileExist fp
                  if exist then do bs <- BS.readFile fp
                                   putStrLn $ "Loading " ++ fp
                                   let Right v = decode bs
                                   return v

                           else do let v = initVisor
                                       game = title . config $ v
                                   putStrLn $ "No visor found at " ++ fp ++", initializing new visor for game " ++ game
                                   return $ v
